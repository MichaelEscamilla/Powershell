    ...
    foreach ($DSInfo in $CatalogItem.DSInfo) {
        if ($DSInfo.OS -like "Windows PE*") {
            if ((Get-Date $DSInfo.Released) -gt (Get-Date 2024.01.01)) {
                ...
                $DestinationPath = "C:\OSDLibrary\Drivers\WinPEDrivers-amd64"
                $RepoPath = "C:\OSDLibrary\Drivers\WinPEDrivers-amd64"
                
                & $PSScriptRoot\New-LenovoDriverPackWinPEV2.ps1 -DSInfo $DSInfo -Family $Family -DestinationPath $DestinationPath -RepoPath $RepoPath
            }
        }
    }
}