...
function Get-CMApplicationsIconDetails {
	param (
		...
		[string]$App_LocalizedDisplayName
	)
	...
	$Query = "LocalizedDisplayName='$App_LocalizedDisplayName' and IsExpired='False' and IsLatest='True'"
...
# Get Application 'Like'
$Applications = Get-CMApplication -Name "*$($ApplicationName)*"

foreach ($App_Found in $Applications) {
	Write-Host "############ $($App_Found.LocalizedDisplayName) ################" -ForegroundColor Magenta

	#Get application icon details
	Get-CMApplicationsIconDetails -App_LocalizedDisplayName "$($App_Found.LocalizedDisplayName)"
...
bU0eFn1TcVJAdZ1TWa/XxTQ+IH6d7e1tmRPK4HjzRTL/O2b4BUErcBXsnNaeAAAAAElFTkSuQmCC')
		...
		Set-CMApplication -Name $App_Found.LocalizedDisplayName -IconLocationFile $dummyIconPath
	...
	try {
		...
		Write-Host "Updating Distribution Points..."
		Update-CMDistributionPoint -ApplicationName "$($App_Found.LocalizedDisplayName)" -DeploymentTypeName "$((Get-CMDeploymentType -ApplicationName "$($App_Found.LocalizedDisplayName)").LocalizedDisplayName)" 
		Write-Host "Successfully Updated the Distribution Points" -ForegroundColor Green
	}
	catch {
